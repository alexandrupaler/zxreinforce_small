...
class ZXCalculus():
    ...
    def get_observation_mask(self) -> tuple:
        ...
        context_features = np.array([count_down, n_spider, n_red / n_spider, n_green / n_spider, n_hada / n_spider,
                                     n_zero / n_spider, n_pi / n_spider, n_arb / n_spider, n_edges, minigame],
                                    dtype=np.float32)
...
def get_mask_list_minigame(colors, angles, selected_node, source, target, selected_edges) -> list:
    ...
    return [copy.copy(node_mask), mask_stop_unmerge_rule, copy.copy(node_mask), copy.copy(node_mask),
            copy.copy(node_mask),
            copy.copy(node_mask), copy.copy(edge_mask), mask_color_edge, copy.copy(edge_mask), copy.copy(edge_mask),
            copy.copy(edge_mask),
...
def get_mask_list(colors, angles, selected_node, source, target,
    ...
    if dont_allow_stop:
        if np.any(np.concatenate(
                [mask_start_unmerge_rule, mask_stop_unmerge_rule, mask_color_change, mask_hadamard_split,
                 mask_hadamard_merge, mask_euler,
    ...
    return [mask_start_unmerge_rule, mask_stop_unmerge_rule, mask_color_change, mask_hadamard_split,
            mask_hadamard_merge, mask_euler,
            mask_merge_rule, mask_color_edge, mask_pi_sym, mask_c_right,
